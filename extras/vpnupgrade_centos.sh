...
Error: Libreswan version '$SWAN_VER' is not supported.
  ...
  3.19-3.23, 3.25-3.27, 3.29-3.30 and 3.31
EOF
    exit 1
    ;;
esac
...
if [ "$SWAN_VER" = "3.29" ] || [ "$SWAN_VER" = "3.30" ] || [ "$SWAN_VER" = "3.31" ]; then
cat <<'EOF'
...
EOF
if [ "$SWAN_VER" = "3.30" ] || [ "$SWAN_VER" = "3.31" ]; then
